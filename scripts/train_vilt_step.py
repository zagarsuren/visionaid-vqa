...
class TrainEvalCallback(TrainerCallback):
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        # Logs contains training metrics such as loss.
        if logs is not None:
            train_record = {
                "step": state.global_step,
                "training_loss": logs.get("loss")
            }
            self.step_train_metrics.append(train_record)
            print(f"Step {state.global_step} - Training metrics (log): {logs}")
        return control

    def on_evaluate(self, args, state, control, metrics, **kwargs):
        # Compute training metrics on the training dataset.
        ...
        print(f"Step {state.global_step} - Training metrics (eval on train set): {train_metrics}")
        return control
...
def main():
    ...
    if train_eval_callback.step_eval_metrics:
        ...
        plt.figure()
        plt.plot(eval_steps, train_loss_eval, marker="o", label="Training Loss (eval on train set)")
        plt.xlabel("Global Step")
        plt.ylabel("Loss")
        plt.title("Training Loss over Steps (Evaluated on Train Set)")
        plt.legend()
        plt.savefig(os.path.join(outputs_dir, "train_loss_eval_plot_steps.png"))
        plt.close()

        plt.figure()
        plt.plot(eval_steps, train_accuracy_eval, marker="o", label="Training Accuracy (eval on train set)")
        plt.xlabel("Global Step")
        plt.ylabel("Accuracy")
        plt.title("Training Accuracy over Steps (Evaluated on Train Set)")
        plt.legend()
        plt.savefig(os.path.join(outputs_dir, "train_accuracy_plot_steps.png"))
        plt.close()

        print(f"Evaluation plots saved in directory: {outputs_dir}")
    
    # Plot Training Loss over Steps from logging events.
    if train_eval_callback.step_train_metrics:
        train_steps = [m["step"] for m in train_eval_callback.step_train_metrics]
        train_loss = [m["training_loss"] for m in train_eval_callback.step_train_metrics]

        plt.figure()
        plt.plot(train_steps, train_loss, marker="o", label="Training Loss (log)")
        plt.xlabel("Global Step")
        plt.ylabel("Loss")
        plt.title("Training Loss over Steps (Logged)")
        plt.legend()
        plt.savefig(os.path.join(outputs_dir, "train_loss_log_plot_steps.png"))
        plt.close()

        print(f"Training log plots saved in directory: {outputs_dir}")
